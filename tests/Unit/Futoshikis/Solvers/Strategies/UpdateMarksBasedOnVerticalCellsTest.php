...
class UpdateMarksBasedOnVerticalCellsTest extends TestCase
{
    public function testApplyDoesNotAddMarksToCells()
    {
        ...
        $this->assertEquals([], $grid->getCell(new GridPosition(1, 0))->getMarks());
        $this->assertEquals([], $grid->getCell(new GridPosition(2, 0))->getMarks());
        $this->assertEquals([], $grid->getCell(new GridPosition(3, 0))->getMarks());
